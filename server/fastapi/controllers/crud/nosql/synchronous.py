...
from database.dal.nosql.synchronous import DataAccessLayer, DataAccessLayerCached
...
class Controller(BaseController):
    ...
    def add_list_endpoint(self):
        ...
        def list(page: Optional[int] = 1, limit: Optional[int] = 100) -> JSONResponse:
            content = self.dal.read_all()
    ...
    def add_create_endpoint(self):
        ...
        def create(data: self.SchemaCreate = Body(...)) -> JSONResponse:
            data = jsonable_encoder(data)
            content = self.dal.create(data)
    ...
    def add_read_endpoint(self):
        ...
        def read(id: ObjectID) -> JSONResponse:
            if not (content := self.dal.read_1(id)):
    ...
    def add_delete_endpoint(self):
        ...
        def delete(id: ObjectID) -> Response:
            if not self.dal.delete(id):
                raise HTTPException(status.HTTP_404_NOT_FOUND, self._404_detail(id))
