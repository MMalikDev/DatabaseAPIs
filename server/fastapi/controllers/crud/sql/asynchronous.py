...
from database.dal.sql.asynchronous import DataAccessLayer, DataAccessLayerCached
...
class Controller(BaseController):
    ...
    def add_list_endpoint(self) -> None:
        ...
        async def list(
            *, db: AsyncSession = Depends(get_db), page: int = 1, limit: int = 100
        ) -> JSONResponse:
            content = await self.dal.read_all(db, page, limit)
    ...
    def add_create_endpoint(self) -> None:
        ...
        async def create(
            *, db: AsyncSession = Depends(get_db), data: self.SchemaCreate
        ) -> JSONResponse:
            if not (content := await self.dal.create(db, data)):
    ...
    def add_read_endpoint(self) -> None:
        ...
        async def read(*, db: AsyncSession = Depends(get_db), id: ID) -> JSONResponse:
            if not (content := await self.dal.read_1(db, id)):
    ...
    def add_update_endpoint(self) -> None:
        ...
        async def update(
            *, db: AsyncSession = Depends(get_db), id: ID, data: self.SchemaUpdate
        ) -> JSONResponse:
            data = data.model_dump()
            if not (content := await self.dal.update(db, id, data)):
                raise HTTPException(status.HTTP_404_NOT_FOUND, self._404_detail(id))
            return JSONResponse(jsonable_encoder(content), status.HTTP_200_OK)
    ...
    def add_delete_endpoint(self) -> None:
        ...
        async def delete(*, db: AsyncSession = Depends(get_db), id: ID) -> Response:
            if not await self.dal.delete(db, id):
                raise HTTPException(status.HTTP_404_NOT_FOUND, self._404_detail(id))
